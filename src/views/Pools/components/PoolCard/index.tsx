...
const PoolCard: React.FC<{ pool: DeserializedPool; account: string }> = ({ pool, account }) => {
  ...
  const isCakePool = earningToken.symbol === 'DATA' && stakingToken.symbol === 'DATA'
    ...
    >
      <PoolCardHeader isStaking={accountHasStakedBalance} isFinished={isFinished && sousId !== 0}>
        <PoolCardHeaderTitle
          ...
          subTitle={isCakePool ? t('Stake DATA, Earn DATA') : t('Stake %symbol%', { symbol: stakingToken.symbol })}
        />
